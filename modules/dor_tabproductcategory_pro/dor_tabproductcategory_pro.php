...
class Dor_Tabproductcategory_Pro extends Module {
    ...
    public function hookDorTabPro($params){
        ...
        if(count($cateInfo) > 0){
            foreach ($cateInfo as $key => $cate) {
                ...
                $cate['thumbCate'] = "";
                /*if($cateCheck != ""){
                    $cateThumbPath = "/c/".$cate['id_category'].".jpg";
                    $linkRewriteCate = $cate['id_category']."_".$cate['link_rewrite'];
                    $images = DorImageBase::renderThumbProduct($cateThumbPath,$linkRewriteCate,$thumbWidthCate,$thumbHeightCate,$quanlity_image);
                    $cate['thumbCate'] = $images;
                }
                else{
                    $cate['thumbCate'] = "";
                }*/
        ...
        return $this->display(__FILE__, 'dor_tabproductcategory_pro.tpl', $this->getCacheId());
    }
    ...
    public function DorCacheTabCategoryPro($tabChose, $cateID, $productTabslider){
        ...
        $fileCache = 'HomeTabCategoryProCaches-Shop'.$id_shop.'-tabchose'.$tabChose.'-'.$cateID;
    ...
    public function hookAjaxCall($params)
    {
        ...
        if(isset($_POST['cateID']) && $_POST['cateID'] != ""){
            ...
            $varsData = $this->display(__FILE__, '/views/templates/hook/product-item.tpl', $this->getCacheId());
        }
